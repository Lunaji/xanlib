...
@pytest.mark.parametrize(
    ...
    [
        f
        for f in (Path(__file__).parent / "test_data/models").rglob("*")
        if f.is_file() and f.suffix.casefold() == ".xbf"
    ],
)
def test_xbf_full_roundtrip(xbf_file, tmp_path):
    scene = load_xbf(xbf_file.resolve())
    temp_file = tmp_path / xbf_file.relative_to(
        Path(__file__).parent / "test_data/models"
    )
